 ...
 */
export const ENV_COLLECTOR_VERSION = "env-2026.08.31-1";
...
Asset: ${assetName}   (modality: PET/CT)
Generated by the admin panel -- do not commit this file to a public repo,
...
SETUP -- ONE ASSET
  ...
  sudo usermod -aG dialout ${serviceUser}
...
def read_zones(client):
    ...
    for unit_id, key, label in ZONES:
        ...
        if rr is None or rr.isError() or len(getattr(rr, "registers", [])) < 2:
            print(f"[nm-env-gateway] {label} (unit {unit_id}) did not answer")
            continue
