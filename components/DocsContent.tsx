    ...
    <div id="main-content" className="min-h-screen p-6 md:p-10" role="main">
      <header className="mb-6">
        ...
        <p className="text-sm text-[var(--text-muted)] leading-relaxed mt-2 max-w-2xl">
          How the {infra.introOwner} fleet is set up and maintained — from imaging a fresh Raspberry Pi
      ...
      <div className="lg:grid lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-10">
        ...
        <div className="min-w-0 flex flex-col gap-12">
          ...
          {/* ---------------- Pi server scripts ---------------- */}
          <Section id="pi-server" title={`Scripts on the Pi server (${infra.serverHostname})`}>
            <P>
              The Pi server — hostname <code className="doc-code">{infra.serverHostname}</code>,{" "}
              <code className="doc-code">{infra.serverLanIp}</code> on the LAN or{" "}
              <code className="doc-code">{infra.serverTailscaleIp}</code> over Tailscale — runs one
              collector per monitored asset as a <b>systemd service</b>. Each asset is a matching pair
              owned by the <code className="doc-code">{infra.piUser}</code> user: a{" "}
          ...
          <Section id="commands" title="Everyday Raspberry Pi commands">
            ...
            <Callout variant="note" title="MagMon runs under systemd, not cron">
              ...
              <code className="doc-code">systemctl status {infra.servicePrefix}</code>, not{" "}
